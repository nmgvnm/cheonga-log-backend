...
def get_groups(db: Session, user_id: int):
    group_ids = db.query(GroupMember.group_id).filter(GroupMember.user_id == user_id).scalar_subquery()
...
def delete_group(db: Session, group_id: int, user_id: int) -> bool:
    group = db.query(Group).filter(Group.id == group_id, Group.owner_id == user_id).first()
    if not group:
        return False

    schedule_ids = [
        s.id for s in db.query(GroupSchedule).filter(GroupSchedule.group_id == group_id).all()
    ]
    if schedule_ids:
        db.query(GroupScheduleAttendee).filter(GroupScheduleAttendee.group_schedule_id.in_(schedule_ids)).delete(synchronize_session=False)
        db.query(GroupScheduleAvailability).filter(GroupScheduleAvailability.group_schedule_id.in_(schedule_ids)).delete(synchronize_session=False)
        db.query(GroupSchedule).filter(GroupSchedule.group_id == group_id).delete(synchronize_session=False)

    db.query(Meeting).filter(Meeting.group_id == group_id).delete(synchronize_session=False)
...
def get_members(db: Session, group_id: int):
    user_ids = db.query(GroupMember.user_id).filter(GroupMember.group_id == group_id).scalar_subquery()
